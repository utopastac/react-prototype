...
//
// Generic handler functions for manipulating the list of dropped components in the admin UI.
 ...
 * handleDelete
 *
 * Removes the selected component from the dropped list.
...
export function handleDelete(params: HandlerParams) {
  ...
  const newDropped = params.dropped.filter((_, i) => i !== params.selectedIdx);
  const newSelectedIdx = params.selectedIdx > 0 ? params.selectedIdx - 1 : null;
  params.dispatch({ type: 'UPDATE', payload: { dropped: newDropped } });
...
export function handleDuplicate(params: HandlerParams) {
  if (params.selectedIdx === null || !params.dropped[params.selectedIdx]) return;
  const item = params.dropped[params.selectedIdx];
  const newDropped = [...params.dropped];
  newDropped.splice(params.selectedIdx + 1, 0, {
    ...item,
    props: { ...item.props },
  });
  params.dispatch({ type: 'UPDATE', payload: { dropped: newDropped } });
 ...
 * handleMoveUp
 *
 * Moves the selected component up by one position in the dropped list.
...
export function handleMoveUp(params: HandlerParams) {
  ...
  const newDropped = [...params.dropped];
  [newDropped[params.selectedIdx - 1], newDropped[params.selectedIdx]] = [newDropped[params.selectedIdx], newDropped[params.selectedIdx - 1]];
  params.dispatch({ type: 'UPDATE', payload: { dropped: newDropped } });
  params.setSelectedIdx(params.selectedIdx - 1);
}
 ...
 * handleMoveDown
 *
 * Moves the selected component down by one position in the dropped list.
 * After moving, the moved component remains selected.
 */
export function handleMoveDown(params: HandlerParams) {
  if (params.selectedIdx === null || params.selectedIdx >= params.dropped.length - 1) return;
  const newDropped = [...params.dropped];
  [newDropped[params.selectedIdx + 1], newDropped[params.selectedIdx]] = [newDropped[params.selectedIdx], newDropped[params.selectedIdx + 1]];
  params.dispatch({ type: 'UPDATE', payload: { dropped: newDropped } });
  params.setSelectedIdx(params.selectedIdx + 1);
 ...
 *
 * Selects the previous component in the dropped list.
...
export function handleSelectPrevious(params: HandlerParams) {
  if (params.selectedIdx === null) {
    ...
    if (params.dropped.length > 0) {
      params.setSelectedIdx(params.dropped.length - 1);
    }
 ...
 * handleSelectNext
 *
 * Selects the next component in the dropped list.
...
export function handleSelectNext(params: HandlerParams) {
  ...
  } else if (params.selectedIdx < params.dropped.length - 1) {
