...
This document describes the layout system that extends the existing single-phone admin interface to support multiple phone previews with independent state management, drag-and-drop, and prop editing.

## Overview

The layout system allows users to create, edit, and manage multiple phone layouts simultaneously. Each layout has its own independent state, including:
...
#### 1. LayoutContext (`src/builder/LayoutContext.tsx`)
...
#### 2. PhonePreview (`src/builder/PhonePreview.tsx`)
...
#### 3. LayoutTabs (`src/builder/LayoutTabs.tsx`)
...
#### 1. useLayoutData (`src/builder/hooks/useLayoutData.ts`)
- **Purpose**: Serialization and restoration of layout state
- **Key Features**:
...
#### 2. useLayoutHistory (`src/builder/hooks/useLayoutHistory.ts`)
- **Purpose**: History management for layout operations
- **Key Features**:
...
import LayoutAdminView from './LayoutAdminView';
...
```tsx
import { useLayoutContext, useActiveLayout } from './LayoutContext';
...
function DataManager() {
  const { exportLayout, importLayout } = useLayoutData();
...
### Using Layout History

```tsx
import { useLayoutHistory } from './hooks/useLayoutHistory';

function HistoryControls() {
  const { handleUndo, handleRedo, canUndo, canRedo } = useLayoutHistory();
...
- The layout system now uses a grid model for layout positions.
...
### Migration Path
1. **Gradual Migration**: Start with single layout, add layout as option
...
4. **State Migration**: Provide tools to convert single to layout
...
├── PhonePreview.tsx           # phone preview component
├── LayoutTabs.tsx                  # Layout switching UI
├── LayoutAdminView.tsx        # Sample integration
├── hooks/
│   ├── useLayoutData.ts       # layout serialization
│   └── useLayoutHistory.ts    # layout history management
...
1. **Layout Not Updating**
   ...
   - Verify the component is wrapped in LayoutProvider
...
This layout system is part of the Interventions Hub React project and follows the same licensing terms. 