/**
 * Formblocker Container Component
 * 
...
export interface FormblockerData {
  ...
  sections: FormblockerSection[]; // Array of sections to render in order
  tabBackground?: TabBackgroundMode; // Background color for the tab (WHITE, GREY, BRAND, BLACK)
}

/**
 * Props interface for the Formblocker component
 */
export interface FormblockerProps {
  data: FormblockerData; // All configuration data for the layout
}
 ...
 */
const Formblocker = ({ data }: FormblockerProps) => {
  ...
  const sectionsRender = sections.map((section, index) => {
    ...
    return createElement(component, { ...props, key: `FormblockerSection${index}` });
  });
